...
def run_step_by_step_migrations(
    cfg: Config, start_rev: str | None = None, target_rev: str = 'head'
) -> list[str]:
    """
    Executa as migrações uma por uma até o target, garantindo rastreabilidade
    caso alguma revisão falhe.
    """
    script = ScriptDirectory.from_config(cfg)
    ordered_revs = list(
        reversed(list(script.walk_revisions(start_rev or 'base', target_rev)))
    )
    executed = []
    for rev in ordered_revs:
        try:
            command.upgrade(cfg, rev.revision)
            executed.append(rev.revision)
        except Exception as exc:
            msg = f"Falha na migração '{rev.revision}' ({rev.doc}): {exc}"
...
def test_alembic_step_by_step_plan():
    ...
    # Garante que cada revisão pode ser resolvida passo a passo
...
async def test_search_materialized_views_lifecycle_and_concurrent_refresh(
    ...
    # 0. Garante que o banco está na revisão pai antes do ciclo
    ...
    # Atualiza o snapshot da sessão para enxergar as alterações do Alembic
...
if __name__ == '__main__':
    print('==> Validando topologia e DAG do Alembic...')
    config = get_alembic_config()
    test_alembic_single_head()
    test_alembic_revisions_chain_integrity()
    test_alembic_step_by_step_plan()
    print('==> Sucesso! Todas as verificações de migração passaram.')
