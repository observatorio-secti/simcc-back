"""create search materialized views

Revision ID: e7796887be76
Revises: 9b38fbff8df4
Create Date: 2026-09-21 19:25:00.000000
...
def upgrade() -> None:
    # 0. Extensões e Configuração de busca textual pt_unaccent
    ...
    # 1. Camada 1: MVs por fonte (granularidade: 1 documento por linha)
    # 1.1 Articles
    op.execute("""
    ...
    SELECT
        ...
        p.title,
        NULLIF(regexp_replace(coalesce(p.development_year, ''), '\\D', '', 'g'), '')::int AS year_,
        NULL::text AS abstract,
    ...
    # 2. Camada 2: MV agregada mv_researcher_search (granularidade: 1 pesquisador por linha)
    op.execute("""
