...
from simcc.v1.ai.providers.base import EmbeddingsProvider
...
class AISearchService:
    ...
    async def search_researchers_hybrid(
        ...
        if researcher_ids:
            ...
            if researcher_name:
                tokens = [
                    t.strip() for t in researcher_name.split() if t.strip()
                ]
                for tok in tokens:
        ...
        r_ids = [
            researcher.id
            for _, researcher, _ in rows
            if hasattr(researcher, 'id')
        ]
